...
class SwiggyAccessibilityScraper : PlatformScraper {
    override val platformId: String = "swiggy"
    override val platformName: String = "Swiggy"
    override val targetPackage: String = "in.swiggy.android"

    override fun getDeepLinkUri(query: String): Uri {
        val encoded = try { URLEncoder.encode(query, "UTF-8") } catch (_: Exception) { query }
        return Uri.parse("swiggy://search?query=$encoded")
    }
    ...
    override suspend fun executeScrape(
        ...
        if (!isAppInstalled(context)) {
            return@withContext ScrapePlatformResult.Failure(
                ScrapeFailure(
                    ...
                    message = "Swiggy is not installed on this device."
                )
            )
        }
        ...
        if (service == null) {
            return@withContext ScrapePlatformResult.Failure(
                ScrapeFailure(
                    ...
                    message = "Accessibility permission required to inspect Swiggy search results."
                )
            )
        }
        ...
        while (elapsed < timeoutMs) {
            ...
            val parsed = parseSwiggyNodes(nodeTexts, query, maxBudget)
        ...
        if (items.isNotEmpty()) {
            ...
            ScrapePlatformResult.Failure(
                ScrapeFailure(
                    ...
                    message = "No matching items found within criteria on Swiggy."
                )
            )
        }
    }

    private fun parseSwiggyNodes(nodes: List<String>, query: String, maxBudget: Double?): List<ProductListingItem> {
